...
const CouponsListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllCoupon(user?.token)
      .then((response) => {
  ...
  useEffect(() => {
    const filteredUsers = coupon.filter((coupon) => {
      const searchTermLowerCase = searchTerm.toLowerCase();
      return coupon.name.toLowerCase().includes(searchTermLowerCase);
    });
  ...
  const handleDeleteRole = (id) => {
    swal({
      title: "Delete",
      text: "Are you sure you want to delete the coupon permanently?",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((willDelete) => {
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteCoupon(user?.token, id)
          .then((res) => {
            setIsLoading(false);
  ...
  const columns = [
    ...
    { header: "Discount Rate", accessor: "discount_rate" },
    { header: "Expiry", accessor: "expiry_date" },
    { header: "Total Usage", accessor: "maximum_uses" },
    { header: "Current Usage", accessor: "current_uses" },
  ];

  const filteredUsers = coupon.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-coupon"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("coupon-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start flex-wrap mt-10 w-[150%] lg:w-full">
              ...
              <div className="bg-[#f4f2f3] py-8 px-12 w-full">
                <TableListing
                  ...
                  handleEditItem={true}
                  handleShowItem={true}
                  editLink="show_coupon"
                  idKey="id"
                  editPermissionKey="coupon-edit"
                  stateKey="coupon_id"
                  startSerialNumber={1 + pagee_offset}
                  showLink={"coupon-details"}
                />
              </div>
