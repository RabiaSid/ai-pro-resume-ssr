...
const ExperienceListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllExperience(user?.token)
      .then((response) => {
  ...
  useEffect(() => {
    const filteredUsers = experience.filter((experience) => {
      const searchTermLowerCase = searchTerm.toLowerCase();
      return experience.job_position
        .toLowerCase()
  ...
  const handleDeleteExperience = (id) => {
    swal({
      title: "Delete",
      text: "Are you sure you want to delete the Experience permanently?",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((willDelete) => {
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteExperience(user?.token, id)
          .then((res) => {
            setIsLoading(false);
  ...
  const filteredUsers = experience.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
    return user.job_position.toLowerCase().includes(searchTermLowerCase);
  });
            ...
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              ...
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  ...
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("experience-example-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start flex-wrap mt-10 w-[150%] lg:w-full">
              ...
              <div className="bg-[#f4f2f3] py-8 px-12 w-full">
                <TableListing
                  ...
                  module="education"
                  handleDeleteItem={handleDeleteExperience}
                  handleEditItem={true}
                  handleShowItem={false}
                  editLink="update-experience"
                  idKey="id"
                  editPermissionKey="experience-example-edit"
                  stateKey="experience_id"
