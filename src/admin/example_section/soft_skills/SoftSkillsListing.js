...
const SoftSkillsListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllSoftSkills(user?.token)
      .then((response) => {
  ...
  useEffect(() => {
    const filteredUsers = softskills.filter((softskills) => {
      const searchTermLowerCase = searchTerm.toLowerCase();
      return softskills.name.toLowerCase().includes(searchTermLowerCase);
    });
  ...
  const handleDeleteSoftSkills = (id) => {
    swal({
      title: "Delete",
      text: "Are you sure you want to delete the Soft Skill permanently?",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((willDelete) => {
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteSoftSkills(user?.token, id)
          .then((res) => {
            setIsLoading(false);
  ...
  const filteredUsers = softskills.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              ...
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-soft-skills"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("soft-skill-example-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start flex-wrap mt-10 w-[150%] lg:w-full">
              ...
              <div className="bg-[#f4f2f3] py-8 px-12 w-full">
                <TableListing
                  ...
                  module="softskills"
                  handleDeleteItem={handleDeleteSoftSkills}
                  handleEditItem={true}
                  handleShowItem={false}
                  editLink="update-soft-skills"
                  idKey="id"
                  editPermissionKey="soft-skill-example-edit"
                  stateKey="softskills_id"
                  startSerialNumber={1 + pagee_offset}
                />
              </div>

              <div className="w-full flex-wrap flex justify-end items-center text-[#0072b1] text-2xl py-4 cursor-pointer ">
                {cp > 1 ? (
                  <BiLeftArrowAlt
                    size={26}
