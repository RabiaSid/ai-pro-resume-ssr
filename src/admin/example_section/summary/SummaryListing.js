...
const SummaryListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllSummary(user?.token)
      .then((response) => {
  ...
  useEffect(() => {
    const filteredUsers = summary.filter((summary) => {
      const searchTermLowerCase = searchTerm.toLowerCase();
      return summary.title.toLowerCase().includes(searchTermLowerCase);
    });
  ...
  const handleDeleteSummary = (id) => {
    swal({
      title: "Delete",
      text: "Are you sure you want to delete the Summary permanently?",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((willDelete) => {
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteSummary(user?.token, id)
          .then((res) => {
            setIsLoading(false);
  ...
  const filteredUsers = summary.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              ...
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-summary"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("summary-example-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start flex-wrap mt-10 w-[150%] lg:w-full">
              ...
              <div className="bg-[#f4f2f3] py-8 px-12 w-full">
                <TableListing
                  ...
                  handleDeleteItem={handleDeleteSummary}
                  handleEditItem={true}
                  handleShowItem={false}
                  editLink="update-summary"
                  showLink=""
                  idKey="id"
                  editPermissionKey="summary-example-edit"
                  stateKey="summary_id"
                  startSerialNumber={start + pagee_offset}
                />
              </div>
