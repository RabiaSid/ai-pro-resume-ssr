...
const EducationListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllEducation(user?.token)
      .then((response) => {
  ...
  useEffect(() => {
    const filteredUsers = education.filter((education) => {
      const searchTermLowerCase = searchTerm.toLowerCase();
      return education.institution.toLowerCase().includes(searchTermLowerCase);
    });
  ...
  const handleDeleteEducation = (id) => {
    swal({
      title: "Delete",
      text: "Are you sure you want to delete the Education permanently?",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((willDelete) => {
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteEducation(user?.token, id)
          .then((res) => {
            setIsLoading(false);
  ...
  const columns = [
    { header: "Institute", accessor: "institution" },
    { header: "Field", accessor: "field" },
    { header: "Grade Type", accessor: "grade_type" },
    { header: "Grade", accessor: "grade" },
  ];

  const filteredUsers = education.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
    return user.institution.toLowerCase().includes(searchTermLowerCase);
  });
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              ...
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-education"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("education-example-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start flex-wrap mt-10 w-[150%] lg:w-full">
              ...
              <div className="bg-[#f4f2f3] py-8 px-12 w-full">
                <TableListing
                  ...
                  module="education"
                  handleDeleteItem={handleDeleteEducation}
                  handleEditItem={true}
                  handleShowItem={false}
                  editLink="update-education"
                  idKey="id"
                  editPermissionKey="education-example-edit"
                  stateKey="education_id"
