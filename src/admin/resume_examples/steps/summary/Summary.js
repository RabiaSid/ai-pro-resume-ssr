...
const Summary = ({
  ...
  const handleStepData = (e) => {
    ...
    ApiService.storeResumeExamplesData(user?.token, resumeData)
      .then((res) => {
        setIsloading(false);
  ...
  useEffect(() => {
    ...
    if (job_positions) {
      ...
      setJob_positions_opt(job_positionsData);
    }
  }, [
    certificate_examples,
    education_examples,
  ...
  const setJobPositionsData = (array) => {
    ...
    setResumeData((prev) => ({ ...prev, job_position: itemIds }));
  };
  ...
  const handleCreateSummary = (e) => {
    ...
    ApiService.createSummary(user?.token, data)
      ...
      .catch((err) => {
        setIsloading(false);
        setErrorsSummary(Object.values(err.response.data.errors));
      });
  };

  // Create Curtifacate
  const [errorsCertificate, setErrorsCertificate] = useState(null);
  ...
  const handleCreateCertificate = (e) => {
    ...
    ApiService.createCertificate(user?.token, data)
      ...
      .catch((err) => {
        setIsloading(false);
        console.log(err);
        setErrorsCertificate(Object.values(err.response.data.errors));
      });
  };
  // Education
  ...
  const [errorsExperience, setErrorsExperience] = useState(null);
  const handleCreateExperience = (e) => {
    ...
    ApiService.createExperience(user?.token, data)
      ...
      .catch((err) => {
        setIsloading(false);
        setErrorsExperience(Object.values(err.response.data.errors));
      });
  };

  // Soft Skill
  const [softSkillName, setSoftSkillName] = useState("");
  const [errorsSoftSkills, setErrorsSoftSkills] = useState(null);
  const handleCreateSoftSkills = (e) => {
    ...
    ApiService.createSoftSkills(user?.token, data)
      ...
      .catch((err) => {
        setIsloading(false);
        setErrorsSoftSkills(Object.values(err.response.data.errors));
      });
  };

  // Tech Skills
  const [techSkillName, setTechSkillName] = useState("");
  const [errorsTeachSkills, setErrorsTeachSkills] = useState(null);
  const handleCreateTechSkills = (e) => {
    ...
    ApiService.createTechSkills(user?.token, data)
      ...
      .catch((err) => {
        setIsloading(false);
        setErrorsTeachSkills(Object.values(err.response.data.errors));
      });
  };
  // Job Position
  ...
  const [errorsJobPosition, setErrorsJobPosition] = useState(null);
  const handleCreateJobPosition = (e) => {
    e.preventDefault();
    ...
    ApiService.createJobPosition(user?.token, data)
      ...
      .catch((err) => {
        setIsloading(false);
        setErrorsJobPosition(Object.values(err.response.data.errors));
      });
  };
    ...
    <div>
      ...
      <Box sx={{ width: "100%" }}>
                ...
                <div className=" ">
                  <div className="py-2 w-full flex flex-col gap-2">
                    ...
                    <div className="relative w-[400px] rounded-full">
                      <img
                        src={
                          resumeData.preview_image
                            ? URL.createObjectURL(resumeData.preview_image)
                            : PlaceholderImage
                        }
                        alt="img"
        ...
        <CustomTabPanel value={value} index={5}>
          <form action="#" onSubmit={handleCreateJobPosition}>
            <div>
              {/* Blog Inputs */}
              <div className="">
