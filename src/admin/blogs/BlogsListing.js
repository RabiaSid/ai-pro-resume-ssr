...
const BlogsListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllBlogs(user?.token)
      .then((response) => {
        setBlogs(response.data.data.blogs);
        console.log(response.data.data.blogs);
        updatePagination(response.data.data.blogs);
      })
  ...
  useEffect(() => {
    const filteredUsers = blogs.filter((blog) => {
      ...
      return blog.name.toLowerCase().includes(searchTermLowerCase);
    });
  ...
  const handleDeleteBlog = (id) => {
    swal({
      ...
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteBlog(user?.token, id)
          .then((res) => {
            setIsLoading(false);
  ...
  const filteredUsers = blogs.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-blog"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("blog-create") ? "flex" : "hidden"
                    }`}
                >
                  CREATE BLOG
                </Link>
