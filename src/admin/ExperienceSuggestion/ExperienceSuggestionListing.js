...
const ExperienceSuggestionListing = () => {
  ...
  const updatePagination = (SuggestionData) => {
    const pages = [];
    const page_total = Math.ceil(SuggestionData.length / 8);
  ...
  useEffect(() => {
    const filteredSuggestion = Suggestion.filter((Suggestion) => {
      ...
      return (
        Suggestion.detail.toLowerCase().includes(searchTermLowerCase) ||
        Suggestion.job_positions.some((job) =>
          job.name.toLowerCase().includes(searchTermLowerCase)
        )
      );
    });
    updatePagination(filteredSuggestion);
    set_cp(1);
    set_pagee_offset(0);
    set_pagee_limit(8);
  }, [searchTerm]);
  ...
  const handleDeleteRole = (id) => {
    swal({
      title: "Delete",
      text: "Are you sure you want to delete the Suggestion permanently?",
      icon: "warning",
      dangerMode: true,
      buttons: true,
    }).then((willDelete) => {
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteExperenceSuggestion(user?.token, id)
          .then((res) => {
            setIsLoading(false);
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-ex-suggestion"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("experience-suggestion-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
            ...
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start flex-wrap mt-10 w-[150%] lg:w-full">
              ...
              <div className="bg-[#f4f2f3] py-8 px-12 w-full">
                <TableListing
                  ...
                  editLink="show-ex-suggestion"
                  showLink={""}
                  idKey="id"
                  editPermissionKey="experience-suggestion-edit"
                  stateKey="Suggestion_id"
...
export default ExperienceSuggestionListing;
