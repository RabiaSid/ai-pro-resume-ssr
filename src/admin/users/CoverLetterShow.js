...
const CoverLetterShow = ({ personalData }) => {
  ...
  const [previewImage, setPreviewImage] = useState(
    personalData.length > 0
      ? global.imageUrl + personalData[0].template?.image
      : ""
  );
  const [doc_id, setDoc_id] = useState(
    personalData.length > 0 ? personalData[0].id : ""
  );
  // Total pages
  ...
  const handleDeleteCover = (id) => {
    swal({
      ...
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteUserCoverAdminSide(user?.token, id)
          .then((res) => {
            setIsLoading(false);
            getData();
            swal({
  ...
  const handleCopyClick = () => {
    const textToCopy =
      global.localPath + "share/" + doc_id + "?share=coverletter";
    navigator.clipboard
  ...
  const share_email = () => {
    if (email_share === "") {
      ...
      const article = {
        link: global.localPath + "share/" + doc_id + "?share=coverletter",
        email: email_share,
        type: "cover letter",
      };
  ...
  const share_doc = async () => {
    ...
    setFile(global.localPath + "share/" + doc_id + "?share=coverletter");
    // console.log(
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody>
            {Array.isArray(paginatedData) && paginatedData.length > 0 ? (
              paginatedData.map((cover, index) => (
                <tr key={cover.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2 border-b">{index + 1}</td>
                  <td className="px-4 py-2 border-b">{cover.name}</td>
                  <td className="px-4 py-2 border-b">{cover.job_title}</td>
                  ...
                  <td className="px-4 py-2 border-b">
                    ...
                    <button className="text-black hover:text-blue-600 mr-2">
                      <FaTrashAlt onClick={() => handleDeleteCover(cover.id)} />
                    </button>
                    <button
                      className="text-black hover:text-blue-600"
                      onClick={() => {
                        setPreviewImage(
                          global.imageUrl + cover.template?.image ||
                          global.imageUrl + cover.template?.image
                        );
                        setDoc_id(cover.id);
                      }}
