...
const Header = ({
  ...
  const handleStepData = (e) => {
    ...
    Cookies.set("coverLetterExampleStep1", JSON.stringify(coverLetterData), {
      expires: 7,
      path: "/",
    });

    navigate("/admin/cover-examples/create-cover-examples/opener");
  };
  ...
  // set All Cities
  const cityNameValue = coverLetterData.city_id;
  useEffect(() => {
    if (cityNameValue) {
  ...
  // All States
  const stateNameValue = coverLetterData.state_id;
  useEffect(() => {
    if (stateNameValue) {
        ...
        <div className="grid sm:grid-cols-1 lg:grid-cols-2 gap-4 items-center w-full">
          ...
          {/* <div>
            <DropdownWithIdValue
              ...
              value={coverLetterData.state_id}
              options={states}
            />
          </div> */}
          {/* city id */}
