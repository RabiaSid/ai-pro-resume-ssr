...
const Cover_Example_listing = () => {
  const [coverExamplesList, setCoverExamplesList] = useState([]);
  ...
  const loadData = () => {
    ApiService.getAllCoverLetterExamplesTemplates(user?.token)
      .then((response) => {
        setCoverExamplesList(response.data.data);
  ...
  useEffect(() => {
    const filteredUsers = coverExamplesList.filter((coverExamplesList) => {
      const searchTermLowerCase = searchTerm.toLowerCase();
      return coverExamplesList.first_name
        .toLowerCase()
  ...
  const handleDeleteRole = (id) => {
    swal({
      ...
      if (willDelete) {
        setIsLoading(true);
        ApiService.deleteCoverExample(user?.token, id)
          .then((res) => {
            setIsLoading(false);
            loadData();
            swal({
  ...
  const filteredUsers = coverExamplesList.filter((user) => {
    const searchTermLowerCase = searchTerm.toLowerCase();
          ...
          <div className="">
            <div className="flex justify-between items-center flex-wrap mt-[5%]">
              ...
              <div className="flex items-center justify-center bg-[#f4f2f3] rounded-full">
                <Link
                  to="create-cover-examples/header"
                  className={`font_1 bg-[#0072b1] text-white rounded-full px-6 py-3 text-lg ${userPermissions.includes("cover-example-create")
                    ? "flex"
                    : "hidden"
                    }`}
                >
                  CREATE COVER EXAMPLES
                </Link>
                  ...
                  <table className="table-auto w-full text-center text-[#959492] text-md">
                    ...
                    <tbody className="bg-white rounded-xl">
                      {filteredUsers
                        ...
                        .map((cover_example, index_cover_example) => (
                          <tr key={index_cover_example} className="h-[50px]">
                            <td className="w-[10%]">
                              {(cp - 1) * 8 + index_cover_example + 1}
                            </td>
                            <td>{cover_example.first_name}</td>

                            <td>{cover_example.email_address}</td>
                            {/* <td>{cover_example.job_title}</td> */}
                            <td>
                              {cover_example.job_positions.map((item2) => (
                                <div>{item2.name}</div>
                              ))}
                            </td>
                            ...
                            <td>
                              <div className="w-full flex justify-center items-center gap-4">
                                {/*  */}
                                <Link
                                  to={`show-cover-examples/${cover_example.id}/header`}
                                  title="Edit"
                                  state={{ cover_example_id: cover_example.id }}
                                ...
                                >
                                  <MdDelete
                                    ...
                                    onClick={() =>
                                      handleDeleteRole(cover_example.id)
                                    }
                                  />
                                </Link>
                              </div>
...
export default Cover_Example_listing;
