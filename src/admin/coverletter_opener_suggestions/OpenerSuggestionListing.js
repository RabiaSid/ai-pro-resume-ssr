...
const BlogsListing = () => {
  ...
  const loadData = () => {
    ApiService.getAllCoverLetterOpenerSuggestions(user?.token)
      .then((response) => {
  ...
  useEffect(() => {
    const filteredUsers = suggestions.filter((suggestions) => {
      ...
      return suggestions.body.toLowerCase().includes(searchTermLowerCase);
    });
  ...
  const handleDeleteSuggestion = (id) => {
    swal({
      ...
      if (willDelete) {
        setIsLoading(true);
        ApiService.DeleteCoverLetterOpenerSuddestions(user?.token, id)
          .then((res) => {
            setIsLoading(false);
