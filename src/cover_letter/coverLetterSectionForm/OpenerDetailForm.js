...
export const OpenerDetailForm = (props) => {
  const [OpenerDetailIsUpdating, setOpenerDetailIsUpdating] = useState(false);
  const [OpenerDetailHasSaved, setOpenerDetailHasSaved] = useState(false);
  const [aiModal, setAiModal] = useState(false);
  const [suggestionForAi, setSuggestionForAi] = useState([]);

  const {
    setOpenerDetail,
    OpenerDetailObjectives,
    OpenerDetailObjectives2,
    resumeOpenerDetail,
    OpenerDetailId,
    profileId,
    itSaving,
  ...
  const [selectedOpenerDetailObjectives, setSelectedOpenerDetailObjectives] =
    useState([]);
  const editorRef = useRef(null);

  useEffect(() => {
    const OpenerDetailValue = watch("opener_detail");
    if (OpenerDetailValue === "") {
      setSelectedOpenerDetailObjectives([]);
    }
  }, [watch("opener_detail")]);

  /////***********useEffects Section *************//////
  useEffect(() => {
    const contentOpenerDetail = selectedOpenerDetailObjectives
      .map((item) => item.objective)
      .join(" ");

    setValue("OpenerDetail", contentOpenerDetail);
  }, [selectedOpenerDetailObjectives]);

  useEffect(() => {
    setValue("OpenerDetail", resumeOpenerDetail);
    if (OpenerDetailId) {
      setValue("OpenerDetailId", OpenerDetailId);
    }
    if (profileId) {
  ...
  // ******** funtions section *******//
  // submit
  const submitOpenerDetail = (data) => {
    setOpenerDetailHasSaved(false);
    setResumeIsUpdating(true);
    // if (formHasError) return;
    itSaving(true);

    // update or create new
    if (data.OpenerDetailId) {
      ApiService.resumeOpenerDetailEditSubmit(
        user?.token,
        data.OpenerDetail,
        data.OpenerDetailId
      )
      ...
      ApiService.resumeOpenerDetailSubmit(user?.token, data)
        .then((res) => {
          itSaving(false);
          setOpenerDetailHasSaved(true);
          storePreviewImage();
  ...
  const handleObjectiveClick = (obj) => {
    setSelectedOpenerDetailObjectives((prevState) => {
  ...
  const OpenerDetailDescription = watch("OpenerDetail");
  useEffect(() => {
    OpenerDetailObjectives?.map((sug) => {
      setSuggestionForAi((prev) => [
        ...prev,
        { id: sug?.pivot?.id, suggestion: sug?.detail },
      ]);
    });
    setOpenerDetail(OpenerDetailDescription);
  }, [OpenerDetailDescription]);
  ...
  const getSunEditorInstance = (sunEditor) => {
    ...
    const iframe = sunEditor.iframe;
    if (iframe) {
      const editableDiv = iframe.contentDocument.body;
  ...
  return (
    <>
      ...
      <form
        onSubmit={handleSubmit(submitOpenerDetail)}
      ...
      >
        <div className="bg-white pt-5 sticky -top-1">
          ...
          {/* <Controller
            ...
            control={control}
            rules={{
              required: "OpenerDetail is required",
              validate: {
                minLength: (value) =>
                  value.trim().split(/\s+/).length >= 10 ||
                  "The OpenerDetail must be at least 10 words",
                maxLength: (value) =>
                  value.trim().split(/\s+/).length <= 250 ||
                  "The OpenerDetail must be at most 250 words",
              },
            }}
            render={({ field }) => (
              <TextField
                ...
                error={!!errors.OpenerDetail}
                helperText={errors.OpenerDetail?.message}
              />
            )}
          /> */}
          <Controller
            ...
            rules={{
              ...
              validate: {
                minLength: (value) =>
                  value.trim().split(/\s+/).length >= 10 ||
                  "The body content must be at least 10 words",
                maxLength: (value) =>
                  value.trim().split(/\s+/).length <= 2000 ||
                  "The body content must be at most 2000 words",
              },
            }}
            render={({ field }) => (
              <>
                ...
                <div className="relative">
                  <SunEditor
                    ...
                    setOptions={{
                      ...
                      // attributesWhitelist: {
                      //   span: "id|class|style|data-suggestions",
                      // },
                    }}
