...
const Header = ({ isOpen }) => {
  ...
  const handleChange = (event) => {
    ...
    //setsoft_skills({ ...soft_skills, first_name: event.target.value });
    ...
    console.log(soft_skills.first_name);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      soft_skills.title === "<br>" ||
      soft_skills.title === "" ||
      soft_skills.title === undefined ||
      soft_skills.title === null
    ) {
      $("#title_valid").html("Title Field is Empty");
    } else if (soft_skills.title.trim().length < 3) {
    ...
    if (title_valid === 1 && valid_desc === 1) {
      ...
      if (page_uuid && isExampleTrue === null) {
        if (is_data === 1) {
          ...
          await axios
            .post(global.baseurl + "/soft_skills/" + soft_skills.id, article, {
              headers,
            })
            .then((data) => {
              if (data) {
          ...
          await axios
            .post(global.baseurl + "/soft_skills", article, { headers })
            .then((data) => {
              if (data) {
                Cookies.set("res_per_soft_skills", 10, { expires: 1 }); // Expires in 1 day
                set_check_loader(0);
                window.location.href =
                  global.localPath + "resume/certificates-details/";
              }
          ...
          .then((data) => {
            if (data) {
              ...
              if (isExampleTrue === "true") {
                ...
                window.location.href = global.localPath + "resume/certificates";
              }
            }
          })
          .catch((err) => {
  ...
  useEffect(() => {
    ...
    if (isExampleTrue === "true") {
      ...
      axios
        .get(global.baseurl + "/soft_skills", { headers })
        .then((data) => {
          if (data) {
      ...
      axios
        ...
        .then((data) => {
          if (data) {
            ...
            setsoft_skills(data.data.data.example.soft_skills[0]);

            setnewDescriptions((prevDesc) => ({
              ...prevDesc,
              name: data.data.data.example.soft_skills
                ?.map((exp) => exp.name)
      ...
      axios
        .get(global.baseurl + "/soft_skills/" + page_uuid, { headers })
        .then((data) => {
          if (data) {
          ...
          <div className="lg:w-full mt-20">
            ...
            <div className="flex flex-wrap justify-between align-start mt-20">
              ...
              <input
                ...
                id="id"
                className="h-8 border border-gray-300 text-gray-500 mt-2 rounded px-4 w-full"
                defaultValue={soft_skills.id ? soft_skills.id : ""}
              ...
              <div className="w-[90%] m-auto mt-10">
                <div className="lg:w-[50%]">
                  ...
                  <input
                    ...
                    id="title"
                    className="h-8 border border-gray-300 text-gray-500 mt-2 rounded px-4 w-full"
                    defaultValue={soft_skills.title ? soft_skills.title : ""}
                    onChange={handleChange}
                    placeholder="Title"
                  />
                </div>
                  ...
                  <a
                    href={
                      isExampleTrue === "true"
                        ...
                        "?example=true"
                        : Number(doc_uuid) === ""
                          ? global.localPath + "resume/certificates"
                          : global.localPath + "resume/certificates-details"
                    }
