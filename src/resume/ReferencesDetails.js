...
const Header = ({ isOpen }) => {
  ...
  const edit = (id) => {
    if (isExampleTrue === "true") {
      window.location.href =
        global.localPath + "resume/references/" + id + "?example=true";
    } else {
      window.location.href = global.localPath + "resume/references/" + id;
    }
  };
        ...
        axios
          ...
          .then((data) => {
            if (data) {
              ...
              axios
                .get(global.baseurl + "/user_references/" + profile_id, {
                  headers,
                })
                .then((data) => {
                  if (data) {
  ...
  useEffect(() => {
    ...
    //GET PERSONAL INFORMATION
    axios
      .get(global.baseurl + "/user_references/" + profile_id, { headers })
      .then((data) => {
        if (data) {
          if (Number(data.data.data.length) !== 0) {
            Cookies.set("res_per_references", 10, { expires: 1 }); // Expires in 1 day
          }
          console.log(data.data.data);
          setpersonal_info(data.data.data);
          set_check_loading(1);
      ...
      <div id="main_contents" className="">
        <div className="flex flex-wrap justify-center items-start min-h-[1000px] px-2 lg:px-6">
          ...
          <div className="lg:w-[30%] my-10 lg:my-40">
            <div className="w-[70%] h-[300px] overflow-hidden m-auto border border-slate-300 p-2">
              <Templates
                my_page={"steps"}
                zoom={isMobile ? 28 : 40}
