...
const Header = ({ isOpen }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      ...
      $("#awards_title_valid").html("Awards Title is Empty");
    } else {
      $("#awards_title_valid").html("");
      valid_title = 1;
    }

    if (
      personal_info.body === "" ||
      personal_info.body === undefined ||
      personal_info.body === null
    ) {
    ...
    if (valid_desc === 1 && valid_title === 1 && valid_body === 1) {
      ...
      if (page_uuid) {
        const article = {
          ...
          body: personal_info.body,
          description: personal_info.description,
          _method: method,
        };
        await axios
          .post(global.baseurl + "/awards/" + page_uuid, article, { headers })
          .then((data) => {
            if (data) {
              set_check_loader(0);
              window.location.href =
                global.localPath +
                "resume/honors-and-awards-details/" +
                profile_id;
            }
          })
          .catch((err) => {
        ...
        await axios
          .post(global.baseurl + "/awards", article, { headers })
          .then((data) => {
            if (data) {
        ...
        <div className="flex flex-wrap justify-center align-top min-h-[1000px] px-2 lg:px-6">
          ...
          <div className="w-full lg:w-[70%]">
            ...
            <div className="flex flex-wrap justify-between align-center mt-20">
              ...
              <div className="w-full xl:w-[50%] px-4 py-2">
                <label htmlFor="body" className="text-md text-gray-500">
                  AUTHORIZATION BODY{" "}
                  <span className="text-rose-600 font-bold">*</span>
                </label>
                <input
                  ...
                  placeholder="Google"
                  className="h-8 border border-gray-300 rounded px-4 w-full mt-2"
                  defaultValue={personal_info.body ? personal_info.body : ""}
                  onChange={handleChange}
                />
                <div
                  className="text-rose-600 text-sm font-semibold"
              ...
              <div className="w-full xl:w-full px-4 py-2">
                ...
                <textarea
                  ...
                  name="description"
                  autoComplete="on"
                  placeholder="Description about your award"
