...
const Header = ({ isOpen }) => {
  ...
  const sel_template = (id, is_paid) => {
    ...
    // if(user.max_res_templates === user.current_res_usage){
    //   console.log(1);
    // }else{
    //   console.log(0);
    // }
    ...
    if (Number(paid) === 1) {
      if (user2.max_res_templates === user2.current_res_usage) {
        ...
        if (Number(id) === 1) {
          ...
          window.location.href = global.localPath + "resume/header";
        } else {
  ...
  return (
    <section className="" id="maindiv">
      <div id="main_contents" className="lg:w-[100%] lg:ml-[0%]">
        <section className="flex justify-center lg:justify-between items-start lg:w-[93%] m-auto flex-wrap min-h-[820px] pt-20">
            ...
            {my_resumes.map((my_resumes, index_my_resumes) => (
              ...
              >
                ...
                <div
                  ...
                  className="get_temp border-4 cursor-pointer relative shadow-[0px_0px_10px_0px_rgba(0,0,0,0.3)] hover:scale-[105%] w-full my-6 h-[300px] lg:h-[400px] overflow-hidden"
                >
                  <Templates
                    zoom={isMobile ? 30 : 50}
                    my_page={"temp"}
