 ...
 <img src="${Step5}" alt="step5" />`,
    ...
    if (e === "duplicate") {
              ...
              axios
                ...
                .then((data) => {
                  if (data) {
                    ...
                    axios
                      .get(
                        global.baseurl + "/personal_information/" + doc_uuid,
                        {
                          headers,
                        }
                      )
      ...
      swal({
        ...
        if (willDelete) {
          ...
          swal("Unfortunately, Your " + resumeData.resume_name + " file was not deleted.");
        }
      });
    }
  ...
  useEffect(() => {
    ...
    ApiService.getAllJobPositions(user?.token)
      .then((res) => {
        console.log("job_positions", res.data.data);
  ...
  useEffect(() => {
    ...
    if (!newResumeId) {
      if (resumeSelectedTemplateId) {
        ...
        const data = {
          resume_name: firstName,
          first_name: firstName,
          last_name: lastName ? lastName : "",
          email_address: user?.email,
          job_title: "Your Job Title",
          phone_number: user?.contact ? user?.contact : "",
          contact_number: "",
        ...
        ApiService.resumeHeaderSubmit(user?.token, data)
          .then((res) => {
    ...
    if (!isExample) {
      ...
      ApiService.showProfileResumeData(
        ...
        .then((res) => {
          ...
          if (res.data.data.technical_skills) {
            per_technical_skills = 10;
          }
          let per_soft_skills = 0;
          if (res.data.data.soft_skills) {
            per_soft_skills = 10;
          }
          let per_experiences = 0;
          if (res.data.data.experiences.length) {
            per_experiences = 10;
          }
          let per_education = 0;
          if (res.data.data.education.length) {
            per_education = 10;
          }
          let per_awards = 0;
          if (res.data.data.awards.length) {
            per_awards = 10;
          }
          let per_certificates = 0;
          if (res.data.data.certificates.length) {
            per_certificates = 10;
          }
          ...
          let calculate_percentage =
            ...
            per_user_languages +
            per_summary;
          setResumePercentage(calculate_percentage);
          ApiService.updateResumePercentage(
            user?.token,
            res.data.data.id,
          ...
          setResumeData({
            ...
            id_no: id_no,
            job_title: job_title,
            resume_percentage: res.data.data?.resume_percentage,
            // summary
            summary: summary,
      ...
      ApiService.showProfileResumeData(user?.token, resumeExampleId)
        .then((res) => {
          ...
          // Experince
          setUserExperiencesList(res.data.data.experiences);
          console.log("User Exp list", res.data.data.experiences);
          setUserEducationList(res.data.data.education);
          // Technical Skills
          const TechSkillData = res.data.data?.technical_skills;
          const TechnicalSkillsArrayData = TechSkillData?.body
  ...
  const downloadPDF = async () => {
    ...
    if (user3?.purchase_templates?.length) {
      console.log("User Templates:", user3?.purchase_templates);
      console.log("activeTemplate: ", activeTemplate);
      user3?.purchase_templates
        ?.filter((temp) => temp.template_id == activeTemplate)
    ...
    if (ownedTemp == 1) {
      ...
      if (paidTemplate) {
        console.log("user3.current_res_usage:", user3.current_res_usage);
        console.log("user3.max_res_templates:", user3.max_res_templates);
  ...
  const handleCheckboxChange = (checked) => {
    setIsChecked(checked); // Update the state in the parent
  };
  ...
  return (
    <section className="bg-[#161616] h-full" id="maindiv">
      ...
      <div className="flex items-center justify-between px-6">
        <Link to={"/"} className="flex">
          <img src={Logo} alt="My Image" width={140} className="logo py-2" />
        </Link>
        <div className="flex gap-1 lg:gap-4 items-center font-Lexend">
          <div style={{ width: "42px", height: "42px" }}>
            <CircularProgressbar
              value={ResumePercentage ? ResumePercentage : 0}
              text={ResumePercentage ? ResumePercentage + "%" : 0 + "%"}
              styles={buildStyles({
                rotation: 0,
      ...
      <div id="main_contents" className="">
        <section className="flex justify-center items-start  flex-wrap px-6 mt-10 py-10 w-full">
          ...
          <div className="w-full">
            ...
            <div className="w-full flex flex-wrap justify-between lg:gap-4 relative">
              ...
              <div className="hidden lg:flex flex-wrap flex-row-reverse justify-end items-start rounded-md sticky top-[80px] h-fit">
                ...
                <div className="hidden lg:flex flex-row-reverse w-fit bg-primary-blue py-6 px-2 ml-4 rounded-full">
                  {/* Tabs */}
                  <div className={"flex flex-col items-center gap-4"}>
                    {toolbarResumeTabs.map((tab) => (
                      ...
                      >
                        <div className="absolute right-0 z-20 group-hover:flex hidden justify-start px-2 rounded-full bg-primary-blue w-[200px] items-center h-full ">
                    ...
                    >
                      <div
                        ...
                        className="absolute right-0 z-20 group-hover:flex hidden justify-start px-2 rounded-full bg-primary-blue w-[200px] items-center h-full "
                      >
                ...
                >
                  <div
                    ...
                    className={`hidden lg:block  2xl:right-0 absolute rounded-xl min-w-[250px] max-w-[600px] right-0 min-h-[150px] max-h-[700px] overflow-y-auto bg-white shadow-lg `}
                  >
                    <div className={``}>
                      {activeToolBarTab === 1 && (
                        <div>
                          <div className="" id="formsSection">
                            ...
                            >
                              ...
                              {activeForm === "experience_resume" && (
                                <ExperienceForm
                                  jobPositionsListData={job_positions}
                                  userCurrentJobPosition={resumeData?.job_title}
                                  countries={countries}
                            ...
                            <div className="grid grid-cols-2 gap-4 p-2">
                              {resumeTemplatesListing?.map((template, idx) => (
                                ...
                                >
                                  {template.is_paid === 1 && (
                                    <div
                                      className="flex bg-gradient-to-r from-[#01B2AC] to-[#0072B1] w-[150px] h-8 absolute left-[-35px] top-16 text-white -rotate-45 justify-center items-center z-20"
                                      style={{
                                        clipPath:
                      ...
                      {activeToolBarTab === 4 && (
                        <div className="w-[400px]">
                          ...
                          <Fonts
                            ...
                            currentFontSize={activeFontSize}
                            userDetails={user3}
                            activeFont={(val) => {
                              setActiveFont(val);
                              changeFormatingResume(
          ...
          >
            <div
              id="scroll_top"
              className={`lg:hidden 2xl:right-0 fixed rounded-xl w-[90%] left-[5%] bottom-10 z-10 max-h-[600px] overflow-y-auto bg-[#fff] shadow-lg pb-10 scroll_top`}
            >
              {/* Buttonsss */}
              <div className="bg-white border-b z-50 w-full sticky top-0 flex justify-between px-2 py-4 ">
                {formOpen ? (
                  <div
                    className="sticky top-[10px] left-[10px] bg-[#0072b1] hover:bg-[#E66868]  w-[30px] h-[30px] text-[#fff] rounded-full flex justify-center items-center cursor-pointer z-30"
                ...
                {activeToolBarTab != 0 ? (
                  <div
                    ...
                    className={` bg-[#E66868] hover:bg-[#E66868] w-[30px] h-[30px] text-[#fff] rounded-full flex justify-center items-center cursor-pointer z-30 `}
                  >
                    {toolbarResumeTabsOpen ? (
                      <RxCross1 size={20} />
                    ) : (
                        ...
                        {resumeTemplatesListing?.map((template, idx) => (
                          ...
                          >
                            ...
                            <h2 className="text-[#0f4d76] text-sm sm:text-md font-Lexend py-2">
                              <strong> {idx + 1}) </strong> {template.name}
                            </h2>
                            <img
                              src={global.imageUrl + template.image}
                              alt={template.name}
