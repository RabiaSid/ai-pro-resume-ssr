...
const Header = ({ isOpen }) => {
  ...
  const goto_add = (id) => {
    if (Number(id) === 0) {
      ...
      // if(Number(digital_signature) === 1){
      //   window.location.href = global.localPath + "resume/digital-signature";
      // }
      // else if(Number(digital_signature) === 0 && Number(custom-section) === 1){
  ...
  const edit = (id) => {
    window.location.href = global.localPath + "resume/languages/" + id;
  };
        ...
        axios
          ...
          .then((data) => {
            if (data) {
              ...
              axios
                .get(global.baseurl + "/user_languages/" + profile_id, {
                  headers,
                })
                .then((data) => {
                  if (data) {
  ...
  useEffect(() => {
    ...
    //GET PERSONAL INFORMATION
    axios
      .get(global.baseurl + "/user_languages/" + profile_id, { headers })
      .then((data) => {
        if (data) {
          console.log(data.data.data);
          setpersonal_info(data.data.data);
        ...
        <div className="flex flex-wrap justify-center items-start min-h-[1000px] px-2 lg:px-6">
          ...
          <div className="w-full lg:w-[70%]">
            ...
            <div className="flex flex-wrap justify-between align-center mt-20">
              <div className="w-full xl:w-full shadow-lg px-4 py-4 border-2 border-slate-300 rounded-xl ">
                        ...
                        >
                          ...
                          {personal_info.map(
                            (personal_info, index_personal_info) => (
                              <Draggable
                                key={personal_info.language}
                                draggableId={personal_info.language}
                                ...
                                {(provided) => (
                                  ...
                                  >
                                    <div className="flex justify-between items-center ">
                                      <div className="lg:w-[40%] font_1 flex justify-start items-center">
                                        <BiGridVertical className="mr-2" />
                                        {personal_info.language
                                          ? personal_info.language
                                          : ""}
                                      </div>
                                      <div className="hidden lg:block lg:w-[40%] font_1">
                                        {personal_info.level
                                          ? personal_info.level
                                          : ""}
                                      </div>
