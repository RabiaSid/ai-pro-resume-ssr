...
const HonorsAndAwardsForm = (props) => {
  ...
  const { user } = useAuth();
  const [honorAndAwardList, setHonorAndAwardList] = useState([]);
  useEffect(() => {
    setHonorAndAwardList(userHonorAndAwardList);
  }, [userHonorAndAwardList]);
  ...
  const submit = (data) => {
    ...
    if (data.award_id) {
      ApiService.resumeAwardsEdit(user?.token, data)
        .then((res) => {
      ...
      ApiService.resumeAwardstore(user?.token, resumeId, data)
        .then((res) => {
          console.log(res);
          reloadTheData();
          isSeving(false);
  ...
  const handleDragEnd = (result) => {
    ...
    const reorderedItems = Array.from(honorAndAwardList);
    const [removed] = reorderedItems.splice(result.source.index, 1);
    reorderedItems.splice(result.destination.index, 0, removed);

    setHonorAndAwardList(reorderedItems);
  ...
  useEffect(() => {
    if (resume_sections) {
      setIsShow(resume_sections.show_awards);
    }
  }, [resume_sections]);
          ...
          <DragDropContext onDragEnd={handleDragEnd}>
            <Droppable droppableId="droppable">
              {(provided) => (
                ...
                >
                  {honorAndAwardList?.map((item, idx) => (
