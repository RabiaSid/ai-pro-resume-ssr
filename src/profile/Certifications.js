...
function Summary({ data, allCountries, reload }) {
  ...
  const submitData = (dataform) => {
    ...
    if (dataform.certificate_id) {
      ApiService.resumeCertificateEdit(user?.token, updatedData)
        .then((res) => {
          console.log(res);
          set_edit_mode(false);
      ...
      ApiService.resumeCertificateStore(user?.token, 0, updatedData)
        .then((res) => {
          console.log(res);
          set_edit_mode(false);
          set_loader_mode(false);
    ...
    <div className="mt-4 font-Lexend">
      <form onSubmit={handleSubmit(submitData)}>
        <div className="flex justify-between items-center text-[#343434] font-bold mb-2">
          Certificates ({String(data.length).padStart(2, "0")})
          {edit_mode ? (
                ...
                <Controller
                  ...
                  rules={{
                    ...
                    validate: {
                      minLength: (value) =>
                        value.length >= 3 ||
                        "The Certificate Title must be at least 3 characters",
                      maxLength: (value) =>
                        value.length <= 100 ||
                        "The Certificate Title must be at most 100 characters",
                    },
                  }}
                  defaultValue=""
                  render={({ field }) => (
