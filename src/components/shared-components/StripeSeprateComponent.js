    ...
    if (error) {
      ...
      if (data.error) {
        ...
        if (cart.template[0].id) {
          ...
          ApiService.Payment(user?.token, submitData)
            .then((res) => console.log(res.data.data))
            .catch((err) => console.log(err));
        }
      } else {
                          ...
                          ApiService.updateUserCoins(
                            ...
                            .then((res) => {
                              ...
                              swal({
                                ...
                                .then(() =>
                                  navigate("/dashboard?tab=mytransactions")
                                )
                                .catch(() =>
                                  navigate("/dashboard?tab=mytransactions")
                                );
            ...
            .then((res) => {
              ...
              const handleStripePaymentConfirm = async () => {
                ...
                if (confirmPayment.error) {
                  ...
                  ApiService.paymentStatus(user?.token, statusUpdateData)
                    .then((res) => {
                      isLoading(false);
                      setLoader(false);
                      swal({
  ...
  return (
    <form onSubmit={handleSubmit(handleStripe)} className="flex flex-col gap-6">
            ...
            render={({ field }) => (
              <div className="relative">
                <input
                  ...
                  id="city"
                  className={`${
                    errors.city ? "border-red-500" : "border-primary"
                  } border-2 py-2 px-2  w-full placeholder:text-[20px] placeholder:text-[#6c757d] text-[20px] focus-visible:outline-none`}
                  placeholder="City"
                />
                {errors.city && (
            ...
            render={({ field }) => (
              <div className="relative">
                <input
                  ...
                  id="state"
                  className={`${
                    errors.state ? "border-red-500" : "border-primary"
                  } border-2 py-2 px-2  w-full placeholder:text-[20px] placeholder:text-[#6c757d] text-[20px] focus-visible:outline-none`}
                  placeholder="state"
                />
                {errors.state && (
            ...
            render={({ field }) => (
              <div className="relative">
                <Select
                  ...
                  styles={errors.country ? customStylesError : customStyles}
                  className={`${
      ...
      <div className="flex justify-center items-center py-8">
        {loader ? (
          <div className="bg-primary text-base font-bold text-white px-20 py-4 rounded-full hover:bg-[#0073b1cc]">
            LOADING...
          </div>
        ) : (
          <button
